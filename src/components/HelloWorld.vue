    ...
    <div class="content">
      <ul v-loading.body="loading">
        <li v-for="(item, index) in msg" :key="item.id"
            :class="[index % 2 === 0?'white':'pink']"
            :title="item.title"
            @click="click(item.id)">
          <i class="el-icon-arrow-right" :class="[index % 2 === 0?'i-pink':'i-white']" style="font-weight: bold;"></i>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.loading = true
      // 取cookie
      this.openSimple = !this.$cookie.get('wcp')
      let url = Host + 'writecomposition'
      this.post(url, '', res => {
        this.loading = false
        if (res.status) {
          this.info = res.info
    ...
    methods: {
      ...
      bClose () {
        ...
        this.$cookie.set('wcp', 'Helloworld', parseInt(this.info.adInfo.interval))
      }
    }
  }
</script>
